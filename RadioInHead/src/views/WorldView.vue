<template>
  ...
  export default {
    ...
    mounted() {
      ...
      fetch('https://nl1.api.radio-browser.info/json/stations/search?limit=100&hidebroken=true&order=clickcount&reverse=true')
        .then(response => response.json())
        .then(data => {
          const germanyRadioStations = data.filter(radio => radio.countrycode === 'DE');
          germanyRadioStations.forEach(station => {
        ...
        .then(data => {
          const englishRadioStations = data.filter(radio => radio.countrycode === 'GB');
          englishRadioStations.forEach(station => {
            const longitude = station.geo_long;
            const latitude = station.geo_lat;
            this.addMarker(longitude, latitude, 0.01);
          });
        ...
        .then(data => {
          const usaRadioStations = data.filter(radio => radio.countrycode === 'US');
          usaRadioStations.forEach(station => {
            const longitude = station.geo_long;
            const latitude = station.geo_lat;
            this.addMarker(longitude, latitude, 0.01);
          });
    ...
    methods: {
      ...
      init() {
        ...
        this.renderer.setSize(window.innerWidth * 0.5, window.innerHeight * 0.6); // Ridotto a metà le dimensioni
